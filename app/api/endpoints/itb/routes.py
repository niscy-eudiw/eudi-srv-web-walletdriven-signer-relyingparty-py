...
from app.services.log_service import add_info_log, add_error_log, mark_successful
...
@with_error_logging("Failed to set up document signing request")
def get_document_for_signature():
    ...
    add_info_log(nonce, "Document Retrieval initialized")

    return jsonify({"qr_code": qr_img_base64, "transaction_id": nonce}), 200

@tests_routes.route("/get/<string:nonce>", methods=["GET"])
@with_error_logging("Relying Party failed to retrieved signed document")
def get_signed_document(nonce):
    response = retrieve_signed_objects(nonce=nonce)
    if response is not None:
        add_info_log(nonce, "Relying Party got signed document")
        mark_successful(nonce)
        return jsonify({ "signed_documents":response}), 200
    else:
        add_info_log(nonce, "Relying Party failed to get signed document")
        return Response("Signed document not received yet.", status=404)

@tests_routes.route("/logs/<string:nonce>", methods=["GET"])
def get_logs(nonce):
    return log_service.get_logs(nonce), 200
